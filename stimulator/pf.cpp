...
vector<vector<string>>pf_matrix;
string IF="IF";
string IDRF="IF";
string EX="IF";
string MEM="IF";
...
void add_row(){
     int rows=pf_matrix.size();
     int cols=pf_matrix[pf_matrix.size()-1].size();
     pf_matrix.resize(rows+1,vector<string>( cols , ""));
}

void add_col(){
    int rows=pf_matrix.size();
    int cols=pf_matrix[rows-1].size();

    for(int i=0;i<pf_matrix.size();i++){
        pf_matrix[i].push_back("");
    }
    
    pf_matrix[0][cols]="CC-"+to_string(cols);
}
...
int stalls_dueto_true_dependency(int pcounter){
    ...
    if(prev_pc >= 0){
         ...
         if((prev_inst.rd== curr_inst.rs1 || prev_inst.rd== curr_inst.rs2 )&& prev_inst.type=="Load_type" && prev_inst.rd!=-1){
             return 1;
         }
    }
...
int stalls_dueto_branch(int pcounter){
    ...
    if(prev_pc >= 0){
        ...
        if(  (prev_inst.rd== curr_inst.rs1 || prev_inst.rd== curr_inst.rs2)  && (prev_inst.rd!=-1)){
            if(prev_inst.type=="R_type" || prev_inst.type=="I_type" || prev_inst.type=="J_type")
             return 1;
             else if(prev_inst.type=="Load_type")
             return 2;
         }
    }
    ...
    if(prev_prev_pc>=0){
       ...
       if((prev_prev_inst.rd== curr_inst.rs1 || prev_prev_inst.rd== curr_inst.rs2) && prev_prev_inst.rd!=-1){
             if(prev_prev_inst.type=="Load_type")
             return 1;
        }
    }
...
bool inst_fetch() {
    ...
    if(pf_matrix.size()>2){
        ...
        int i=pf_matrix.size()-2;
        while(pf_matrix[pf_matrix.size()-2][i]!="IF"){
            if(pf_matrix[pf_matrix.size()-2][i]=="stall"){
                pf_matrix[pf_matrix.size()-1][i]="stall";
            }
            i++;
        }
        i++;
        while(pf_matrix[pf_matrix.size()-2][i]=="stall"){
            if(pf_matrix[pf_matrix.size()-2][i]=="stall"){
                pf_matrix[pf_matrix.size()-1][i]="stall";
            }
            i++;
        }
        while(pf_matrix[pf_matrix.size()-1][i]=="stall"){
            i++;
        }
        while(pf_matrix[pf_matrix.size()-1][i]!=""){
            i++;
        }
       

        if(!is_branch_dependent(pc)){
            pf_matrix[pf_matrix.size()-1][i]="IF";
        }
        else{
            int i=pf_matrix.size()-2;
            
            while(pf_matrix[pf_matrix.size()-2][i]!="ID/RF"){
                if(pf_matrix[pf_matrix.size()-2][i]=="stall"){
                    pf_matrix[pf_matrix.size()-1][i]="stall";
                }
                i++;
    ...
    Lines[pc-1].second.second=inst;
     ...
     if(pf_matrix.size()>2){
        ...
        if( stalls_dueto_branch(pc)==2){
            ...
            i++;
            pf_matrix[pf_matrix.size()-1][i]="ID/RF";
...
bool execute( instruction inst) {
    ...
    if(pf_matrix.size()>2){
        int i=pf_matrix.size()-2;
        while(pf_matrix[pf_matrix.size()-2][i]!="EX"){
            if(pf_matrix[pf_matrix.size()-2][i]=="stall"){
                pf_matrix[pf_matrix.size()-1][i]="stall";
            }
            i++;
        ...
        if(stalls_dueto_true_dependency(pc)==1){
            add_col();
            pf_matrix[pf_matrix.size()-1][i]="stall";
            
            i++;
            pf_matrix[pf_matrix.size()-1][i]="EX";
...
bool memory(instruction inst, int ex_mem) {
    ...
    if(pf_matrix.size()>2){
        int i=pf_matrix.size()-2;
        while(pf_matrix[pf_matrix.size()-2][i]!="MEM"){
            if(pf_matrix[pf_matrix.size()-2][i]=="stall"){
                pf_matrix[pf_matrix.size()-1][i]="stall";
            }
            i++;
        ...
        pf_matrix[pf_matrix.size()-1][i]="MEM";
...
bool write_back(instruction inst, int mem_wb) {
    ...
    if(pf_matrix.size()>2){
        int i=pf_matrix.size()-2;
        while(pf_matrix[pf_matrix.size()-2][i]!="WB"){
            if(pf_matrix[pf_matrix.size()-2][i]=="stall"){
                pf_matrix[pf_matrix.size()-1][i]="stall";
            }
            i++;
        ...
        pf_matrix[pf_matrix.size()-1][i]="WB";
...
void set_intial_pf_matrix(){
    pf_matrix.resize(1,vector<string>(6,""));
    
    for(int j=1;j<=5;j++){
        pf_matrix[0][j]="CC-"+to_string(j);
    }
}

void set_pf_matrix_first(){
    pf_matrix[pf_matrix.size()-1][1]="IF";
    pf_matrix[pf_matrix.size()-1][2]="ID/RF";
    pf_matrix[pf_matrix.size()-1][3]="EX";
    pf_matrix[pf_matrix.size()-1][4]="MEM";
    pf_matrix[pf_matrix.size()-1][5]="WB";
}
...
void parser() {
    ...
    while (pc != -1 && pc<max_pc) {
        ...
        if(pf_matrix.size()==1){
         ...
         pf_matrix[pf_matrix.size()-1][0]=Lines[pc].second.first;
...
float calculateIPC(int a[3]){
    ...
    int tot_stalls=0;
    int tot_inst=0;
    int rows=pf_matrix.size()-1;
    int cols=pf_matrix[rows].size()-1;
    bool isOk=false;
    ...
    for(int j=0;j<pf_matrix[rows].size();j++){
        bool status=false;
        for(int i=0;i<=rows;i++){
            if(pf_matrix[i][j]=="stall"){
                status=true;
            }
        }
        if(status){
...
void display() {
    ...
    for(int i=0;i<pf_matrix.size();i++){
        fout<<"-------------------------\n";
        for(int j=0;j<pf_matrix[i].size();j++){
            if(j==0){continue;}
            fout<<pf_matrix[i][j]<<"          "<<"|"<<"         ";
        }
        fout<<endl;
